...
class StratifiedSampler(RandomCVSampler):
    ...
    '''
    def __init__(self, data, batch_size: int = 128, input_size: int = None,
                 test_split: float = 0.2, validation_split: float = None,
                 distribution: dict = None, random_seed: int = None):
        '''
        Parameters
        ----------
        data
        ...
        random_seed : int
            TODO
        '''
        super().__init__(data, batch_size=batch_size, input_size=input_size,
        ...
        if distribution is None:
            ...
            sample = data if self.input_size < SAMPLE_SIZE_LARGE else \
                self.rnd.choice(data, SAMPLE_SIZE_LARGE, replace=False)
            distribution = {label: len([x for x in sample if x == label]) / len(sample)
    ...
    def indices(self, subset: str = None, batch_size: int = None):
        ...
        while any([len(oversample[label]) < batch_size
                   for label in self.distribution.keys()]):
