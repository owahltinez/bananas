...
class _CVSampler(_BaseSampler):
    def __init__(
        ...
        test_size = int(
            (self.input_size * test_split)
            if self.input_size != sys.maxsize
        ...
        self.validation_size = (
            int(test_size * validation_split)
            if self.input_size > VALIDATION_MIN_SAMPLE_SIZE or validation_set
            else 0
        )
...
class RandomCVSampler(_CVSampler):
    ...
    def __init__(
        ...
        for i in range(iter_max):
            ...
            if idx not in validate_idx:
                test_idx.add(idx)
            if len(test_idx) == self.test_size:
                break
            if i >= iter_max - 1:
