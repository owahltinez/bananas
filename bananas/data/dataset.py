...
class DataSet(object):
    ...
    def __init__(
        ...
        **sampling_kwargs
        ...
        sampling_method : SamplingStrategy, optional
            Strategy to use when sampling this dataset via `input_fn()`
        sampling_kwargs : , optional
        ...
        if not isinstance(features, list) or not all(
            [isinstance(column, Feature) for column in features]
        ):
            raise TypeError(
                "Provided values should be a Feature list. Found unexpected type: "
                "%r" % type(features)
            )
        ...
        for feat in self.features.values():
            assert feat.count == self.count, (
                "Features do not have equal number of samples. "
                "Expected %d, found %d" % (self.count, feat.count)
            )
        if hasattr(self, "target"):
            assert self.target.count == self.count, (
                "Target does not have equal number of "
        ...
        self.categorical = {
            i: col.sample_classes
            for i, col in enumerate(self.features.values())
    ...
    def ix(self, key: (Tuple[List[str], Any], Any), transpose: bool = False):
        ...
        if isinstance(key, tuple):
            names, key = key[0], key[1:]

        # Special case: if names is an "everything" slice (i.e. ":"), return all columns
        if (
            isinstance(names, slice)
            and names.start is None
            and names.stop is None
            and names.step is None
        ):
            names = list(self.features.keys())
    ...
    def from_ndarray(
        ...
        **sampling_kwargs
        ...
        values = [
            Feature(col, name=str(i), sampling_method=sampling_method, **sampling_kwargs)
    ...
    def from_csv(
        ...
        **sampling_kwargs
        ...
        columns = [
            Feature(col, name=name, sampling_method=sampling_method, **sampling_kwargs)
