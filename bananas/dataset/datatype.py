...
class DataType(IntEnum):
    ...
    def parse(data: numpy.ndarray) -> "DataType":
        ...
        if (
            ...
            and any(
                [
                    numpy.issubdtype(data.dtype, dtype)
                    for dtype in (DTYPE_BOOL[1], DTYPE_INT[1], DTYPE_UINT8[1])
                ]
            )
            and numpy.array_equal(data.astype(int).sum(axis=1), numpy.ones(data.shape[0]))
        ...
        if numpy.issubdtype(data.dtype, DTYPE_FLOAT[1]) and numpy.all(
        ...
        if any(
            [
                data.dtype == dtype[0] or numpy.issubdtype(data.dtype, dtype[1])
