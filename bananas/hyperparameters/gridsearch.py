...
def _iter_parameters(learner: Type[Learner], params: Dict[str, Any]):
    ...
    has_steps = issubclass(learner, Pipeline) and valid_parameters(
        learner.__init__, {"steps": None}
    )
    if has_steps:
        steps_iters = [
...
class GridSearch(_MetaLearner):
    ...
    def _get_learner(self, idx, parameters: Dict[str, Any]) -> Learner:
        if idx not in self._learners_cache:
            has_verbose = issubclass(self.learner, Pipeline) and valid_parameters(
                self.learner.__init__, {"verbose": None}
            )
            if has_verbose:
