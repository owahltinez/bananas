...
def check_array(arr: Iterable, min_samples: int = 1, min_dimension: int = 0,
    ...
    if num_samples < min_samples:
        raise ValueError('Input has less than the minimum required number of samples. Expected at '
    ...
    if num_columns < min_dimension:
        raise ValueError('Input has less than the minimum required number of columns. Expected at '
    ...
    if max_dimension is not None and num_columns > max_dimension:
        raise ValueError('Input has more than the maximum allowed number of columns. Expected at '
    ...
    if not allow_nan and numpy.issubdtype(arr.dtype, numpy.number) and \
        (not numpy.isfinite(arr.sum()) or not numpy.isfinite(arr).all()):
    ...
    if arr.dtype == DTYPE_FLOAT[0] and \
        numpy.all(arr == arr.astype(DTYPE_INT[0])):
        arr = arr.astype(DTYPE_INT[0])

    # Convert all int-like types to int, leave uint8 alone
    if arr.dtype != DTYPE_INT[0] and arr.dtype != DTYPE_UINT8[0] \
    ...
    if not any([numpy.issubdtype(arr.dtype, dtype) for dtype in ALLOWED_DTYPES]):
        raise ValueError('Input has non-allowed dtype. Expected one of %r, found %r' %
...
def transpose_array(arr: Iterable):
    ...
    `ndarray`.
    '''
    assert isinstance(arr, ARRAY_LIKE), 'Unknown array-like type used. Expected one of %r, found '\
        '%r' % (ARRAY_LIKE, type(arr))
...
def equal_nested(a: Iterable, b: Iterable, atol: float = 1E-6) -> [bool]:
    ...
    if all([isinstance(x, ARRAY_LIKE) for x in (a, b)]):
        if len(a) != len(b): return [False] * max(len(a), len(b))
...
def concat_arrays(*arrays: Iterable):
    ...
    for arr in arrays:
        ...
        if not isinstance(arr, ARRAY_LIKE):
            raise TypeError('Unknown array type found. Expected one of %r, found %r' %
                            (ARRAY_LIKE, type(arr)))

        # Input shape check
        inner_shape = shape_of_array(arr)[1:]
        if inner_shape != expected_shape:
            raise ValueError('Shape mismatch. Expected all arrays to be of shape %r, found %r' %
...
def array_to_tuple(arr: Iterable):
    ...
    if not isinstance(arr, (list, numpy.ndarray)):
        raise TypeError('Unexpected type of input. Expected one of %r, found %r' %
...
def flatten(arr: Iterable):
    ...
    if not isinstance(arr, ARRAY_LIKE):
        raise TypeError('Function expected type to be one of %r, found %r' %
                        (ARRAY_LIKE, type(arr)))
