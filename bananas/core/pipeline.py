...
class Pipeline(Learner):
    ...
    def __init__(self, steps: List[PipelineStep], verbose: bool = False):
        ...
        for i, step in enumerate(steps):
            ...
            assert isinstance(step, PipelineStep) and issubclass(
    ...
    def _call_method(instance, method_name, *args, **kwargs):
        ...
        kwargs = {
            name: value for name, value in kwargs.items() if name in signature(method).parameters
        }
    ...
    def is_transformer_only(self):
        ...
        for _, step in self.steps.items():
            if not (
                isinstance(step, BaseTransformer)
                or (isinstance(step, Pipeline) and step.is_transformer_only())
            ):
                return False
        return True
    ...
    def _traverse(
        ...
        return (
            check_array(X)
            if inverse or not hasattr(final_estimator, final_method)
            else Pipeline._call_method(final_estimator, final_method, X, y=y)
        )
    ...
    def hyperparameters(dataset: DataSet, steps: Iterable[PipelineStep]):
        ...
        for i, step in enumerate(steps):
            ...
            has_steps = issubclass(step.learner, Pipeline) and valid_parameters(
                step.learner.__init__, {"steps": None}
            )
