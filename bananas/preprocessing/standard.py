    ...
    def _init_steps(
        ...
        if normalization is not None and self.continuous_:
            if not isinstance(normalization, NormalizationStrategy):
                raise ValueError(
                    "Unknown normalizer requested: %s. Expected one of: %r"
                    % (normalization, NormalizationStrategy)
                )
        ...
        if encoding is not None and classes:
            if not isinstance(encoding, EncodingStrategy):
                raise ValueError(
                    "Unknown encoder requested: %s. Expected one of: %r"
                    % (encoding, EncodingStrategy)
                )
            ...
            if encoding == EncodingStrategy.AUTO:
                ...
                drop_columns = [k for k, v in classes.items() if v and len(v) > ONEHOT_MAX_CLASSES]
                encode_columns = {
