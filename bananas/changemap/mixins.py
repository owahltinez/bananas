...
class InputChangeMixin(object):
    ...
    feature set change events. When an object like a transformer modifies the feature set, it is its responsibility to emit the appropriate `ChangeMap` by calling the `on_output_shape_changed`
    function with the computed change map.

    Conversely, classes inheriting from `InputChangeMixin` should override the `on_input_shape_changed`
    function to appropriately handle changes in input. By default, the function raises
    `UnexpectedShapeError`, which is very undesirable! Classes implementing proper change map handling
    should try to avoid raising an error at all costs, even if that means losing some of the previous
    work. For example, a neural network may reset all of its weights so it can begin learning the data
    anew, rather than throwing an error. In a situation like that, raising a warning would be more
    appropriate.
    '''
