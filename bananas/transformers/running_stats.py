...
class RunningStats(ColumnHandlingTransformer):
    ...
    def fit(self, X):
        ...
        for i, col in enumerate(X):
            ...
            if len(shape) > 1: col = flatten(col)
            ...
            for val in col:
                prev_mean = self.mean_.get(i, 0.)
                self.count_[i] = self.count_.get(i, 0) + 1
                self.mean_[i] = prev_mean + (val - prev_mean) / self.count_[i]
                self._delta_squared_[i] = self._delta_squared_.get(i, 0.) + \
                    (val - self.mean_[i]) * (val - prev_mean)

            self.variance_[i] = self._delta_squared_[i] / self.count_[i]
            self.stdev_[i] = self.variance_[i] ** .5
    ...
    def print_stats(self):
        ...
        for col in self.columns_.keys():
            print('%d\t%s' % (col, '\t'.join(['%.03f' % getattr(self, stat)[col] for stat in stats])))
        print()
