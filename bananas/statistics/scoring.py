...
def score_r2(y_true: List[float], y_pred: List[float]) -> float:
    ...
    if almost_zero(tse):
        return 0. if almost_zero(sse) else 1.
...
def roc_curve_binary(y_true: List[float], y_prob: List[float],
    ...
    for idx_bin in range(bin_count):
        ...
        threshold = 1. - idx_bin / float(bin_count)
        ...
        tpr[idx_bin] = \
            counts.true_positives / max(1, counts.true_positives + counts.false_negatives)

        # False Positive Rate = False Positives / (True Negatives + False Positives)
        fpr[idx_bin] = \
            counts.false_positives / max(1, counts.true_negatives + counts.false_positives)
...
def score_auroc(y_true: List[int], y_prob: List[List[float]]) -> float:
    ...
    tpr_means = [(tpr[i] + tpr[i - 1]) / 2. for i in range(1, len(tpr))]
